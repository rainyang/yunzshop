...
if ($operation == 'display') {
    ...
    $log     = pdo_fetch('select * from ' . tablename('sz_yi_store_withdraw') . ' w inner join ' . tablename('sz_yi_store') . ' s on w.store_id = s.id where w.id=:id and w.uniacid=:uniacid limit 1', array(
        ':id' => $id,
        ':uniacid' => $uniacid
    ));
    if (empty($log)) {
    ...
    if ($paytype == 'manual') {
        ...
        // TODO: 要不要给提现的用户发一条通知  要
         $message = array(
                                'keyword1' => array('value' => '门店提现打款通知', 'color' => '#73a68d'),
                                'keyword2' => array('value' => '【商户名称】' . $cashier_stores['name'], 'color' => '#73a68d'),
                                'remark' => array('value' => '您的提现申请已经手动打款完成,请注意查收!')
                            );          
        m('message')->sendCustomNotice($log['openid'], $message);
        plog('cashier.withdraw.withdraw', "门店商户提现 方式: 手动 ID: {$log['id']} <br/>商户信息: ID: {$log['store_id']} / {$log['openid']}/{$log['storename']}/{$log['contact']}/{$log['mobile']}");
        ...
        m('message')->sendCustomNotice($log['openid'], $message);
        plog('cashier.withdraw.withdraw', "门店商户提现 ID: {$log['id']} 方式: 微信 金额: {$log['money']} <br/>商户信息: ID: {$log['store_id']} / {$log['openid']}/{$log['name']}/{$log['contact']}/{$log['mobile']}");
        ...
        // TODO: 要不要给提现的用户发一条通知
        plog('cashier.withdraw.withdraw', "拒绝门店商户提现 ID: {$log['id']} 金额: {$log['money']} <br/>商户信息: ID: {$log['store_id']} / {$log['openid']}/{$log['storename']}/{$log['contact']}/{$log['mobile']}");
