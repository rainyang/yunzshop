    ...
    <div class="w1200 m0a">
        ...
        <div class="rightlist">
                            ...
                            <div class="form-group">
                                <div class="col-sm-8 col-lg-12 col-xs-12">
                                    ...
                                    <div class='input-group'>
                                        ...
                                        {php echo tpl_form_field_daterange('search[time_range]',
                                        array('starttime'=>array_get($requestSearch,'time_range.start',0),'endtime'=>array_get($requestSearch,'time_range.end',0),'start'=>0,'end'=>0),true);}
                ...
                <table class='table'
                    ...
                    <tr class='trhead'>
                        <td colspan='8' style="text-align: left;">
                            订单数: <span id="total">{{$list['total']}}</span> 订单金额: <span id="totalmoney"
                                                                                        style="color:red">{{$total_price}}</span>元&nbsp;@if( $perm_role == 1)
                                结算金额: <span style="color:red">@if( $costmoney>0){{$costmoney}}</span>元 &nbsp;<a
                                        class="btn btn-default"
                                        href="{php echo $this->createWebUrl('order/list',array('applytype'=>1))}">提现</a>@if( !empty($shopset['weixin']))
                                    <a class='btn btn-default' onclick="return confirm('确认微信钱包提现?')"
                                       href="{php echo $this->createWebUrl('order/list',array('applytype'=>2));}">微信提现</a>@endif
                            @else没有可提现金额@endif@endif
                        </td>
                    </tr>
                </table>
                <table class='table'
                       style='float:left;margin-bottom:5px;table-layout: fixed;line-height: 40px;height: 40px'>
                ...
                <table class='table' style='float:left;border:1px solid #ccc;border-top:none;table-layout: fixed;'>
                    ...
                    <tr class='trbody'>
                        <td class="goods_info">
                            <img src="@if( 0&&$order['cashier']==1){{$order['name']['thumb']}}@else{php echo tomedia($order_goods['thumb'])}@endif">
                        </td>
                        ...
                        <td style='border-left:none;text-align:left;/*width:150px*/'>@if( $requestSearch['plugin'] != "fund")
                                原价: {php echo number_format(
                                $order_goods['goods_price']/$order_goods['total'],2)} @endif<br/>应付: {php echo
                        ...
                        @if( $order_goods_index==0)
                            <td rowspan="{php echo count($order['has_many_order_goods'])}">
                                ...
                                <br/>
                                {{$order['belongs_to_member']['realname']}}<br/>{{$order['belongs_to_member']['mobile']}}
                            </td>
                            <td rowspan="{php echo count($order['has_many_order_goods'])}">
                                @if( $order['status'] > 0)
                            ...
                            <td rowspan="{php echo count($order['has_many_order_goods'])}" style='width:18%;'>
                                <table style='width:100%;'>
                                    <tr>
                                        ...
                                        <td style='border:none;text-align:right;;'>￥{php echo number_format(
                                            $order['goods_price'] ,2)}
                                        </td>
                                    </tr>
                                    <tr>
                            ...
                            <td rowspan="{php echo count($order['has_many_order_goods'])}"><label
                                ...
                                'plugin' => $requestSearch['plugin'], 'openid' => $requestSearch['openid']))}">查看详情</a></td>
