...
//    protected $hidden = ['id','uniacid','status','is_deleted','is_member_deleted','created_at','updated_at','deleted_at'];
    ...
    public static function getOrderDetail($orderId)
    {
        ...
        }])->select(['id','uid','order_sn','price','goods_price','create_time','finish_time','pay_time','send_time','cancel_time','dispatch_type_id','pay_type_id','status','refund_id','dispatch_price','deduction_price'])->find($orderId);
        return $orderModels;
    }
}