...
class MemberFavoriteController extends ApiController
{
    ...
    public function isFavorite()
    {
        ...
        throw new \app\common\exceptions\ShopException('未获取到商品ID');
    }
    ...
    public function store()
    {
        if (\YunShop::request()->goods_id) {
            $memberId = \YunShop::app()->getMemberId();
            if (MemberFavorite::getFavoriteByGoodsId(\YunShop::request()->goods_id, $memberId)){
                throw new \app\common\exceptions\ShopException('商品已收藏，不需要重复添加！');
            }
    ...
    public function destroy()
    {
        if (\YunShop::request()->goods_id) {
            ...
            if (!$favoriteModel) {
                throw new \app\common\exceptions\ShopException("未找到记录或已删除");
            }
            if ($favoriteModel->delete()) {
                return $this->successJson("移除收藏成功");
            } else {
                throw new \app\common\exceptions\ShopException("数据写入出错，移除收藏失败");
            }
        }
        throw new \app\common\exceptions\ShopException("未获取到商品ID");
    }
