...
<div class="form-group notice">
    ...
    <div class="col-sm-4">
        ...
        <div class='input-group'>
            <input type="text" name="saler" maxlength="30" value="{if !empty($saler)}{$saler['nick_name']}/{$saler['real_name']}/{$saler['mobile']}{/if}" id="saler" class="form-control" readonly />
            <div class='input-group-btn'>
                <button class="btn btn-default" type="button" onclick="popwin = $('#modal-module-menus-notice').modal();">选择通知人</button>
                <button class="btn btn-danger" type="button" onclick="$('#uid').val('');$('#saler').val('');$('#saleravatar').hide()">清除选择</button>
            </div>
        </div>
...
<div class="form-group">
    ...
    <div class="col-sm-9 col-xs-12">
            ...
            <label class="checkbox-inline">
                <input type="checkbox" value="0" name='widgets[notice][type][]' {if in_array(0,$noticetype)}checked{/if} /> 下单通知
            </label>
            <label class="checkbox-inline">
                <input type="checkbox" value="1" name='widgets[notice][type][]' {if in_array(1,$noticetype)}checked{/if} /> 付款通知
            </label>
            <label class="checkbox-inline">
                <input type="checkbox" value="2" name='widgets[notice][type][]' {if in_array(2,$noticetype)}checked{/if} /> 买家收货通知
            </label>
...
<script language='javascript'>
    ...
    function search_members() {
        ...
        $.get("{php echo $this->createWebUrl('member.member.get-search-member')}", {
