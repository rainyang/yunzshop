...
if ($operation == 'display') {
    ...
    if (!empty($_GPC['displayorder'])) {
        ...
        message('分类排序更新成功！', $this->createPluginWebUrl('recharge/adv',
            array(
                'op' => 'display'
            )
        ), 'success');
    }
    $list = $this->model->getAllAdv();
    ...
    $item = $this->model->getAdvById($id);
} elseif ($operation == 'delete') {
    ca('shop.adv');
    $id   = intval($_GPC['id']);
    $item = $this->model->getAdvById($id);
    if (empty($item)) {
        message('抱歉，幻灯片不存在或是已经被删除！', $this->createPluginWebUrl('recharge/adv',
            array(
                'op' => 'display'
            )
        ), 'error');
    }
    pdo_delete('sz_yi_recharge_adv',
        array(
            'id' => $id
        )
    );
    plog('shop.adv', "删除幻灯片 ID: {$id} 标题: {$item['advname']} ");
    message('幻灯片删除成功！', $this->createPluginWebUrl('recharge/adv',
        array(
            'op' => 'display'
        )
    ), 'success');
