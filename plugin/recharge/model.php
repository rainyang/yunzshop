 ...
 * 		 		 		 		 */
		...
		public function mobile_blance_api($data){
		    ...
		    $blanceurl = 'http://dataplatform.yunzshop.com/api/web/v1/site/blance'; //账户余额查询接口
		    $ch = curl_init();  
		...
		function mobile_submit_api($data){
		    ...
		    global $_W, $_GPC;
		/*  $queryurl = 'http://dataplatform.yunzshop.com/api/web/v1/site/query'; //订单查询接口
		    $submiturl = 'http://dataplatform.yunzshop.com/api/web/v1/site/submit'; //订单提交接口
		    $blanceurl = 'http://dataplatform.yunzshop.com/api/web/v1/site/blance'; //账户余额查询接口
		    $backurl = 'http://dataplatform.yunzshop.com/api/web/v1/site/back';  //回调地址接口
		...
		*/    
		    ...
		    if(!empty($status)){
		        ...
		        //file_put_contents(IA_ROOT."/data_submit_return_log.txt", print_r($status,true),FILE_APPEND);
		        file_put_contents(IA_ROOT."/submit_return.txt", print_r($status,true),FILE_APPEND);
		    }else{
		        $status['createtime'] = date('Y-m-d H:i:s',time());
		        $status['desc'] = '没收到充值接口返回信息...';
		        //file_put_contents(IA_ROOT."/data_submit_return_log.txt", "没收到充值接口返回信息...",FILE_APPEND);
		        file_put_contents(IA_ROOT."/submit_return.txt", print_r($status,true),FILE_APPEND);
		    }
		    ...
		    if (!empty($status['out_order_id']) && $status['result'] == "0") {
		        ...
		        // if ($set['data']['auto_refund'] == 1) {
		            ...
		            if($returnid){
		                ...
		                $isrefund= m("finance")->refund($openid, $data['out_order_id'], $refundno, $refundprice, $refundprice);
		                if($isrefund){
		                    file_put_contents(IA_ROOT."/data_submit_refund_price_log.txt", "订单".$data['out_order_id']."提交失败退款成功...",FILE_APPEND);
