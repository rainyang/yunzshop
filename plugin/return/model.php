	...
	class ReturnModel extends PluginModel
	{
		...
		public function setOrderMoneyReturn($_var_0=array(),$uniacid=''){
			...
			if($r_ordermoney>0 && $queue_count)
			{
				...
				pdo_query("update  " . tablename('sz_yi_return') . " set return_money = return_money + '".$r_each."',last_money = '".$r_each."',updatetime = '".time()."' WHERE uniacid = '". $uniacid ."' and status=0 and (money - `return_money`) > '".$r_each."' and returnrule = '".$_var_0['returnrule']."' ");

				pdo_query("update  " . tablename('sz_yi_return') . " set last_money = money - return_money, status=1, return_money = money, updatetime = '".time()."' WHERE uniacid = '". $uniacid ."' and status=0 and (money - `return_money`) <= '".$r_each."' and returnrule = '".$_var_0['returnrule']."' ");
				...
				$return_record = pdo_fetchall("SELECT sum(r.money) as money, sum(r.return_money) as return_money, sum(r.last_money) as last_money,m.openid  FROM " . tablename('sz_yi_return') . " r 
				 ...
				 WHERE r.uniacid = '". $uniacid ."' and r.updatetime = '".time()."' and r.returnrule = '".$_var_0['returnrule']."'  group by r.mid");
