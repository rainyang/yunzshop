    ...
    class ReturnModel extends PluginModel
    {
                ...
                if ($good['isreturnqueue'] == 1) {
                    ...
                    for ($i = 1; $i <= $good['total']; $i++) {
                        ...
                        if (($queuenum - $return_queue) >= $set['queue']) {
                            ...
                            pdo_update('sz_yi_order_goods_queue', array('returnid' => $queueid, 'status' => '1'),
                                array('id' => $queue['id'], 'uniacid' => $uniacid));
                            //商品排列全返返现
                            ...
                            $messages = array(
                                'keyword1' => array(
                                    'value' => $set['queue_title'] ? $set['queue_title'] : '排列返现通知',
                                    'color' => '#73a68d'
                                ),
                                'keyword2' => array(
                                    'value' => $queue_price_txt ? $queue_price_txt : '本次返现金额' . $queue['price'] . "元！",
                                    'color' => '#73a68d'
                                )
                            );
                    ...
                    $queue_messages = array(
                        'keyword1' => array(
                            'value' => $set['add_queue_title'] ? $set['add_queue_title'] : '加入排列通知',
                            'color' => '#73a68d'
                        ),
                        'keyword2' => array(
                            'value' => $queuemessages_txt ? $queuemessages_txt : "您已加入排列，排列号为" . $queuemessages . "号！",
                            'color' => '#73a68d'
                        )
                    );
        ...
        public function setMembeerLevel($orderid, $set = array(), $uniacid = '')
        {
            ...
            if ($money > 0) {
                ...
                $msg = array(
                    'keyword1' => array(
                        'value' => $set['member_title'] ? $set['member_title'] : '购物返现通知',
                        'color' => '#73a68d'
                    ),
                    'keyword2' => array(
                        'value' => $member_price_txt ? $member_price_txt : '[返现金额]' . $money . '元,已存到您的余额',
                        'color' => '#73a68d'
                    )
                );
        ...
        */
        public function cumulative_order_amount($orderid)
        {
            global $_W, $_GPC;
            $set = $this->getSet();
            $order = pdo_fetch("SELECT * FROM " . tablename('sz_yi_order') . " WHERE id=:id and uniacid=:uniacid",
        ...
        public function setOrderRule($order_goods, $order_price, $set = array(), $uniacid = '')
        {
            ...
            $msg = array(
                'keyword1' => array(
                    'value' => $set['add_single_title'] ? $set['add_single_title'] : '订单全返通知',
                    'color' => '#73a68d'
                ),
                'keyword2' => array(
                    'value' => $order_price_txt ? $order_price_txt : '[订单返现金额]' . $order_price,
                    'color' => '#73a68d'
                )
            );
            $templateid = $set['templateid'];
        ...
        public function setOrderMoneyRule($order_goods, $order_price, $set = array(), $uniacid = '')
        {
            ...
            $msg = array(
                'keyword1' => array(
                    'value' => $set['total_title'] ? $set['total_title'] : '订单金额累计通知',
                    'color' => '#73a68d'
                ),
                'keyword2' => array(
                    'value' => $total_price_txt ? $total_price_txt : '[订单累计金额]' . $return_money,
                    'color' => '#73a68d'
                ),
            ...
            if ($data_money) {
                foreach ($data_money as $key => $value) {
                    if ($value['money'] >= $orderprice) {
                        ...
                        //订单累计金额加入队列
                        pdo_insert('sz_yi_return_tpm', $data);
                        pdo_update('sz_yi_return_money', array('money' => $value['money'] - $orderprice),
                            array('id' => $value['id'], 'uniacid' => $uniacid));
                    }
                }
        ...
        public function setReturnCredits($data = array())
        {
            ...
            if (!empty($data)) {
                ...
                foreach ($data as $key => $value) {
                    ...
                    if (!empty($uid)) {
                        $credit = pdo_fetchcolumn("SELECT " . $value['credit'] . " FROM " . tablename('mc_members') . " WHERE `uid` = :uid",
                            array(
                                ':uid' => $uid
                            ));
                        ...
                        $credit = pdo_fetchcolumn("SELECT " . $value['credit'] . " FROM " . tablename('sz_yi_member') . " WHERE  uniacid=:uniacid and openid=:openid limit 1",
                            array(
                                ':uniacid' => $_W['uniacid'],
                                ':openid' => $value['openid']
                            ));
        ...
        public function setReturnCredit(
            $openid = '',
            $credittype = 'credit1',
            $credits = 0,
            $returntype = 1,
            ...
            if (!empty($uid)) {
                ...
                $value = pdo_fetchcolumn("SELECT {$credittype} FROM " . tablename('sz_yi_member') . " WHERE  uniacid=:uniacid and openid=:openid limit 1",
                    array(
                        ':uniacid' => $_W['uniacid'],
                        ':openid' => $openid
                    ));
        ...
        public function setDelay($set = array(), $uniacid = '')
        {
            if (isset($set['delay']) && $set['delay'] > 0) {
                ...
                $daytimes = 3600 * $days;
                $delay_queue = pdo_fetchall("SELECT * FROM " . tablename('sz_yi_return_tpm') . " where uniacid = :uniacid and status = :status AND create_time + " . $daytimes . " <= unix_timestamp() ",
                    array(':uniacid' => $uniacid, ':status' => '1'));
            } else {
                $delay_queue = pdo_fetchall("SELECT * FROM " . tablename('sz_yi_return_tpm') . " where uniacid = :uniacid and status = :status",
                    array(':uniacid' => $uniacid, ':status' => '1'));
            }
