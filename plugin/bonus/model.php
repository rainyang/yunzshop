...
if (!class_exists('BonusModel')) {
	class BonusModel extends PluginModel
	{
		...
		public function calculate($orderid = 0, $update = true)
		{
			...
			$goods = pdo_fetchall('select og.id,og.realprice,og.goodsid,og.total,og.optionname,g.hascommission,g.nocommission from ' . tablename('sz_yi_order_goods') . '  og ' . ' left join ' . tablename('sz_yi_goods') . ' g on g.id = og.goodsid' . ' where og.orderid=:orderid and og.uniacid=:uniacid', array(':orderid' => $orderid, ':uniacid' => $_W['uniacid']));
		...
		public function getChildAgents($id){
            ...
            $sql = "select id, agentlevel from " . tablename('sz_yi_member') . " where agentid={$id} and status=1 and isagent = 1 and uniacid=".$_W['uniacid'];
		...
		public function getInfo($openid, $options = null){
            ...
            $time             = time();
            $day_times        = intval($set['settledaysdf']) * 3600 * 24;
            $agentcount       = count($this->getChildAgents($agentid));
		...
		public function checkOrderPay($orderid = '0')
		{
			...
			foreach ($bonus_goods as $key => $val) {
				$openid = pdo_fetchcolumn("select openid from " . tablename('sz_yi_member') . " where id=".$mid." and uniacid=".$_W['uniacid']);
				$agent_money = pdo_fetchcolumn("select sum(money) from " . tablename('sz_yi_bonus_level') . " where mid=".$mid." and uniacid=".$_W['uniacid']);
		...
		public function upgradeLevelByAgent($mid)
		{
			...
			if($isleveup == true){
				pdo_update('sz_yi_member', array('bonuslevel' => $levelup['id']), array('id' => $member['id']));
				//查看是否可以连升级
