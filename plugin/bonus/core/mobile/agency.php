...
$af_supplier = pdo_fetch("select * from " . tablename("sz_yi_agency") . " where openid='{$openid}' and uniacid={$_W['uniacid']}");
$template_flag  = 0;
$diyform_plugin = p('diyform');
if ($diyform_plugin) {
    $set_config        = $diyform_plugin->getSet();
    $supplier_diyform_open = $set_config['bonus_diyform_open'];
    if ($supplier_diyform_open == 1) {
        $template_flag = 1;
        $diyform_id    = $set_config['bonus_diyform'];
        if (!empty($diyform_id)) {
...
if ($_W['isajax']) {
    if ($_W['ispost']) {
        if ($template_flag == 1) {
            ...
            pdo_insert('sz_yi_agency',$m_data);
...
if ($template_flag == 1) {
    include $this->template('diyform/agency');
}
