	...
	if (!empty($list)) {
		...
		foreach ($list as &$row) {
			...
			if (!empty($this->set['openorderdetail'])) {
				$goods = pdo_fetchall("SELECT og.id,og.money,g.thumb,g.title,og.total,og.optionname from " . tablename('sz_yi_bonus_goods') . " og" . " left join " . tablename('sz_yi_goods') . " g on g.id=og.ordergoodid  " . " where og.orderid=:orderid and og.uniacid = :uniacid order by og.createtime  desc ", array(':uniacid' => $_W['uniacid'], ':orderid' => $row['id']));
