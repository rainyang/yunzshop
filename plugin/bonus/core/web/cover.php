...
ca('bonus.cover');
$rule = pdo_fetch("select * from " . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => "sz_yi分红中心入口设置"));
if (!empty($rule)) {
...
if (checksubmit('submit')) {
	...
	$rule_data = array('uniacid' => $_W['uniacid'], 'name' => 'sz_yi分红中心入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => intval($data['status']));
