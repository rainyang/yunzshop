...
if ($operation == 'check') {
	...
	if ($_W['ispost'] && $_W['isajax']) {
		...
		if (empty($log)) {
			return show_json(0, '未找到要兑换记录!');
		}
		if (empty($log['status'])) {
			return show_json(0, '无效兑换记录!');
		}
		if ($log['status'] >= 3) {
			return show_json(0, '此记录已兑换过了!');
		}
