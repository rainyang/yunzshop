...
ca('fund.cover');
$rule = pdo_fetch("select * from " . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => "众筹中心入口设置"));
if (!empty($rule)) {
...
if (checksubmit('submit')) {
	...
	$rule_data = array('uniacid' => $_W['uniacid'], 'name' => '众筹中心入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => intval($data['status']));
	...
	$cover_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => $this->modulename, 'title' => trim($data['title']), 'description' => trim($data['desc']), 'thumb' => $data['thumb'], 'url' => $this->createPluginMobileUrl('fund'));
	pdo_insert('cover_reply', $cover_data);
	plog('fund.cover', '修改众筹中心入口设置');
	message('众筹中心中心入口设置成功!', referer(), 'success');
}
load()->func('tpl');
