...
if (checksubmit('submit')) {
    ...
    foreach ($sets as $set) {
        ...
        if ($wechatid == $uniacid || $wechatid == -1) {
            $cs = pdo_fetch('select * from ' . tablename('ewei_shop_system_copyright') . " where uniacid=:uniacid limit 1", array(
                ':uniacid' => $uniacid
            ));
            if (empty($cs)) {
                pdo_insert('ewei_shop_system_copyright', array(
                    'uniacid' => $uniacid,
                    'copyright' => $post,
                    'bgcolor' => $_GPC['bgcolor']
                ));
            } else {
                pdo_update('ewei_shop_system_copyright', array(
                    'copyright' => $post,
                    'bgcolor' => $_GPC['bgcolor']
                ), array(
                    'uniacid' => $uniacid
