...
function copy_plugin_set($identity, $wechatid, $wechatid1, $transtype)
{
    ...
    if (is_array($plugins) && isset($plugins[$identity])) {
        $newset = pdo_fetch("select * from " . tablename('ewei_shop_sysset') . ' where uniacid=:uniacid limit 1', array(
            ':uniacid' => $wechatid1
        ));
        if (empty($newset)) {
            $newplugins = array(
...
if (checksubmit('submit')) {
    ...
    if (is_array($_GPC['shop'])) {
        foreach ($_GPC['shop'] as $data) {
            if ($data == 'goods') {
                ...
                $goods = pdo_fetchall('select * from ' . tablename('ewei_shop_goods') . " where uniacid=:uniacid", array(
                    ':uniacid' => $wechatid
                ));
                foreach ($goods as $g) {
                    $goodsid = $g['id'];
                    ...
                    $params     = pdo_fetchall('select * from ' . tablename('ewei_shop_goods_param') . " where goodsid=:goodsid and uniacid=:uniacid", array(
                        ':uniacid' => $wechatid,
                        ':goodsid' => $goodsid
                    ));
                    foreach ($params as $p) {
                    ...
                    $newspecs = array();
                    $specs    = pdo_fetchall('select * from ' . tablename('ewei_shop_goods_spec') . " where goodsid=:goodsid and uniacid=:uniacid", array(
                        ':uniacid' => $wechatid,
                        ':goodsid' => $goodsid
                    ));
                    ...
                    $options        = pdo_fetchall('select * from ' . tablename('ewei_shop_goods_option') . " where goodsid=:goodsid and uniacid=:uniacid ", array(
                        ':uniacid' => $wechatid,
                        ':goodsid' => $goodsid
                    ));
                ...
                $ccates1 = array();
                $tcates1 = array();
                $pcates  = pdo_fetchall('select * from ' . tablename('ewei_shop_category') . " where uniacid=:uniacid and parentid=0", array(
                    ':uniacid' => $wechatid
                ));
                foreach ($pcates as $pcate) {
                    ...
                    $ccates = pdo_fetchall('select * from ' . tablename('ewei_shop_category') . " where parentid=:parentid ", array(
                        ':parentid' => $pcateid
                    ));
                    foreach ($ccates as $ccate) {
                        ...
                        $tcates     = pdo_fetchall('select * from ' . tablename('ewei_shop_category') . " where parentid=:parentid ", array(
                            ':parentid' => $ccateid
                        ));
                ...
                $goods = pdo_fetchall('select id,pcates,ccates,tcates from ' . tablename('ewei_shop_goods') . " where uniacid=:uniacid", array(
                    ':uniacid' => $wechatid1
                ));
                foreach ($goods as $g) {
    ...
    if (is_array($_GPC['creditshop'])) {
        foreach ($_GPC['creditshop'] as $data) {
            if ($data == 'goods') {
                ...
                $cates        = pdo_fetchall('select * from ' . tablename('ewei_shop_creditshop_category') . " where uniacid=:uniacid", array(
                    ':uniacid' => $wechatid
                ));
                $categoodsids = array();
                foreach ($cates as $cate) {
                    ...
                    $goods     = pdo_fetchall('select * from ' . tablename('ewei_shop_creditshop_goods') . " where uniacid=:uniacid and cate=:cate", array(
                        ':uniacid' => $wechatid,
                        ':cate' => $cateid
                    ));
                    foreach ($goods as $g) {
                ...
                if (!empty($categoodsids)) {
                    $goods = pdo_fetchall('select * from ' . tablename('ewei_shop_creditshop_goods') . " where uniacid=:uniacid and id not in (" . implode(',', $categoodsids) . ")", array(
                        ':uniacid' => $wechatid
                    ));
                    foreach ($goods as $g) {
                        $goodsid = $g['id'];
    ...
    if (is_array($_GPC['virtual'])) {
        foreach ($_GPC['virtual'] as $data) {
            ...
            $cates        = pdo_fetchall('select * from ' . tablename('ewei_shop_virtual_category') . " where uniacid=:uniacid", array(
                ':uniacid' => $wechatid
            ));
            $catevirtuals = array();
            foreach ($cates as $cate) {
                ...
                $newcateid = pdo_insertid();
                $virtuals  = pdo_fetchall('select * from ' . tablename('ewei_shop_virtual_type') . " where uniacid=:uniacid and cate=:cate", array(
                    ':uniacid' => $wechatid,
                    ':cate' => $cateid
                ));
                foreach ($virtuals as $g) {
                    $goodsid        = $g['id'];
                    $datas          = pdo_fetchall('select * from ' . tablename('ewei_shop_virtual_data') . " where uniacid=:uniacid and typeid=:typeid and usetime=0", array(
                        ':uniacid' => $wechatid,
                        ':typeid' => $goodsid
                    ));
            ...
            if (!empty($catevirtuals)) {
                $virtuals = pdo_fetchall('select * from ' . tablename('ewei_shop_virtual_type') . " where uniacid=:uniacid and id not in (" . implode(',', $catevirtuals) . ")", array(
                    ':uniacid' => $wechatid
                ));
                foreach ($virtuals as $g) {
                    $goodsid = $g['id'];
    ...
    if (is_array($_GPC['article'])) {
        foreach ($_GPC['article'] as $data) {
            if ($data == 'article') {
                ...
                $cates        = pdo_fetchall('select * from ' . tablename('ewei_shop_article_category') . " where uniacid=:uniacid", array(
                    ':uniacid' => $wechatid
                ));
                $catearticles = array();
                foreach ($cates as $cate) {
                    ...
                    $newcateid = pdo_insertid();
                    $articles  = pdo_fetchall('select * from ' . tablename('ewei_shop_article') . " where uniacid=:uniacid and article_category=:article_category", array(
                        ':uniacid' => $wechatid,
                ...
                if (!empty($catearticles)) {
                    $articles = pdo_fetchall('select * from ' . tablename('ewei_shop_article') . " where uniacid=:uniacid and id not in (" . implode(',', $catearticles) . ")", array(
                        ':uniacid' => $wechatid
                    ));
                    foreach ($articles as $g) {
                        $goodsid = $g['id'];
    ...
    if (is_array($_GPC['coupon'])) {
        foreach ($_GPC['coupon'] as $data) {
            if ($data == 'coupon') {
                ...
                $cates       = pdo_fetchall('select * from ' . tablename('ewei_shop_coupon_category') . " where uniacid=:uniacid", array(
                    ':uniacid' => $wechatid
                ));
                $catecoupons = array();
                foreach ($cates as $cate) {
                    ...
                    $newcateid = pdo_insertid();
                    $coupons   = pdo_fetchall('select * from ' . tablename('ewei_shop_coupon') . " where uniacid=:uniacid and catid=:catid", array(
                        ':uniacid' => $wechatid,
                        ':catid' => $cateid
                    ));
                ...
                if (!empty($catecoupons)) {
                    $coupons = pdo_fetchall('select * from ' . tablename('ewei_shop_coupon') . " where uniacid=:uniacid and id not in (" . implode(',', $catecoupons) . ")", array(
                        ':uniacid' => $wechatid
                    ));
                    foreach ($coupons as $g) {
                        $goodsid = $g['id'];
