...
if ($_W['isajax']) {
	if ($operation == 'display') {
		...
		$id = intval($_GPC['id']);
		$apply = pdo_fetch('select * from ' . tablename('sz_yi_store_withdraw') . ' where id=:id and `store_id`=:storeid and uniacid=:uniacid limit 1', array(':id' => $id, ':storeid' => $storeid, ':uniacid' => $uniacid));
		if (empty($apply)) {
		...
		$pindex = max(1, intval($_GPC['page']));
		$psize = 5;
		$apply = pdo_fetch('select orderids from ' . tablename('sz_yi_store_withdraw') . ' where id=:id and `mid`=:mid and uniacid=:uniacid limit 1', array(':id' => $id, ':mid' => $mid, ':uniacid' => $uniacid));
		if (empty($apply)) {
			show_json(0, array('message' => '未找到提现申请!'));
		}
		$orderids = iunserializer($apply['orderids']);
