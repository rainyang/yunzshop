...
ca('indiana.cover');
$rule = pdo_fetch("select * from " . tablename('rule') . ' where uniacid=:uniacid and module=:module and name=:name limit 1', array(':uniacid' => $_W['uniacid'], ':module' => 'cover', ':name' => "夺宝中心入口设置"));
if (!empty($rule)) {
...
if (checksubmit('submit')) {
	...
	$rule_data = array('uniacid' => $_W['uniacid'], 'name' => '夺宝中心入口设置', 'module' => 'cover', 'displayorder' => 0, 'status' => intval($data['status']));
	...
	$cover_data = array('uniacid' => $_W['uniacid'], 'rid' => $rid, 'module' => $this->modulename, 'title' => trim($data['title']), 'description' => trim($data['desc']), 'thumb' => $data['thumb'], 'url' => $this->createPluginMobileUrl('indiana'));
	pdo_insert('cover_reply', $cover_data);
	plog('indiana.cover', '修改夺宝中心入口设置');
	message('夺宝中心中心入口设置成功!', referer(), 'success');
}
