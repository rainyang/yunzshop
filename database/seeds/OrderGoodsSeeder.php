        ...
        $sourceList = DB::table($this->sourceTable)->chunk(100, function($records){
            foreach ($records as $record){
                ...
                if ($goodInfo){
                    ...
                    if(preg_match('/^http.*/',$record['title'])){
                        $record['thumb'] = $goodInfo['thumb']; //todo 最后的url取决于迁徙后放在哪个路径
                    } else if (preg_match('/^images.*/',$record['title'])){
                        $record['thumb'] = $_SERVER['SERVER_NAME'].'/attachment/'.$goodInfo['thumb']; //todo 最后的url取决于迁徙后放在哪个路径
                    }
                } else {
