...
class CreateImsYzGoodsPrivilegeTable extends Migration {
	...
	public function up()
	{
		Schema::create('yz_goods_privilege', function(Blueprint $table)
		{
			...
			$table->integer('once_buy_limit')->nullable()->comment('每次限购数量');
			$table->integer('total_buy_limit')->nullable()->comment('总共限购数量');
			$table->dateTime('time_begin_limit')->nullable()->comment('限购开始时间');
			$table->dateTime('time_end_limit')->nullable()->comment('限购结束时间');
		});
	}
