...
class RemakePayType extends Migration
{
    ...
    public function up()
    {
        if (Schema::hasTable('yz_pay_type')) {
            \Illuminate\Support\Facades\DB::select('TRUNCATE TABLE `ims_yz_pay_type`');
...
');
        \Illuminate\Support\Facades\DB::select('UPDATE `ims_yz_pay_type` SET `id`=0 WHERE `code` = \'unPay\'');
