...
class CreateImsYzMemberRelationTable extends Migration {
	...
	public function up()
	{
		...
		{
			$table->integer('relation_id', true)->comment('编号');
			...
			$table->integer('become_ordercount')->nullable()->comment('消费x次');
			$table->decimal('become_moneycount', 5)->nullable()->comment('消费x元');
			$table->integer('goods_id')->nullable()->comment('购买的商品');
