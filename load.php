...
function get_loader_location_from_ini($php_ini = '')
{
    ...
    foreach ($lines as $l) {
        if (preg_match("/^\s*$ext_start\s*=\s*\"?([^\"]+)\"?/i",$l,$corr_matches)) {
            if (preg_match("/$loader_name_match/i",$corr_matches[1])) {
                ...
                if (empty($loader_path)) {
                    $errors[ERROR_INI_NOT_FIRST_ZE] = "ionCube Loader 必须在 Zend extension 配置项的最上面, $php_ini.";
                }
            }
        }
        if (empty($loader_path)) {
    ...
    if ($loader_path === '') {
        $errors[ERROR_INI_ZE_LINE_NOT_FOUND] = "在配置文件中没有找到zend_extension, $php_ini.";
...
function loader_download_instructions()
{
    ...
    if (is_ms_windows()) {
        if (is_bool($sysinfo['THREAD_SAFE'])) {
            $download_str = '<li>Windows 下载以下一种文件' . $sysinfo['PHP_COMPILER'];
    ...
    uploading from a local PC to your server.<br>";
        ...
        if ($basename == "") {
            ...
            echo '<li>下载以下一种压缩文件 ' . $loader['osnamequal'] . ' ' . $loader['arch'] . ':'; 
            ...
            echo "<p>请注意, MS Windows 安装板适用于从 Windows PC 上传到你的 ${loader['osname']} 服务器.<br>";
            echo "</p>";
...
function loader_install_instructions($server_type,$loader_dir = '')
{
    ...
    if (SERVER_LOCAL == $server_type) {
        echo "<li>把解压后的文件放到<code>$loader_dir</code>里面</li>";
    } else {
...
function zend_extension_instructions($server_type,$loader_dir)
{
    ...
    if (SERVER_SHARED == $server_type && own_php_ini_possible()) {
        ...
        if (empty($sysinfo['PHP_INI_DIR'])) {
            echo "<li>编辑这个文件<code>${sysinfo['PHP_INI']}</code>";
        } else {
    ...
    if ($editing_ini) {
        echo " 并且在其他{$kwd}上面放置一下内容:<br>";
        foreach ($zend_extension_lines as $zl) {
            echo "<code>$zl</code><br>";
        }
        if (!empty($php_ini_path)) {
            if (zend_extension_line_missing($php_ini_path)) {
                //echo "<a>或者, replace your current <code>$php_ini_path</code> file with <a href=\"$base&amp;page=phpconfig&amp;ininame=$php_ini_name&amp;stype=$server_type_code&amp;download=1&amp;prepend=1\">this new $php_ini_name file</a>."; 
                echo "<a>或者用<a href=\"$base&amp;page=phpconfig&amp;ininame=$php_ini_name&amp;stype=$server_type_code&amp;download=1&amp;prepend=1\">这个$php_ini_name文件</a>替换你当前的<code>$php_ini_path</code> 文件. ."; 
            }
        }
    }
    echo '</li>';
...
function server_restart_instructions()
{
    ...
    if ($sysinfo['SS']) {
        echo "<li>重启${sysinfo['SS']}服务.</li>";
    } else {
        echo "<li>重启web服务.</li>";
    }

    echo "<li>web服务重启后, <a href=\"$base&amp;page=loader_check\" onclick=\"showOverlay();\">点击这里测试</a>.</li>";

    if ($sysinfo['SS'] == 'Apache' && !is_ms_windows()) {
        echo '<li>如果安装失败,请联系客服.</li>';
    }
}
...
function php_ini_install($server_type_desc = null, $server_type = SERVER_DEDICATED, $required = true)
{
    ...
    if ($required) {
        echo "你可以使用 <code>$php_ini_name</code> 配置文件.";
    } else {
...
function initial_page()
{
    ...
    <html>
    <head>
        <title>ionCube Loader 安装向导</title>
...
{
    echo '<p><strong>为了安全,请成功后删除此文件load.php.</strong></p>';
}

function contact_script_provider_instructions()
{
    echo '<p>如果您运行有问题,请联系商城.</p>';
}
...
function loader_already_installed($rtl = null)
{
    ...
    echo '<h4>安装成功</h4>';
    if ($newer_version) {
        echo '<p>ionCube Loader 版本' . $lv . $php_str . ' <strong>已经安装</strong> 但是是老的版本.';
        ...
        echo '<p>ionCube Loader版本' . $lv . $php_str . ' 已经成功安装.</p>'; 
        echo '</div>';
...
function loader_not_installed()
{
    ...
    if ($host_type != SERVER_UNKNOWN && is_array($loader) && !$sysinfo['DEBUG_BUILD']) {
        ...
        if (!empty($errors)) {
            if (count($errors) > 1) {
                $problem_str = "请注意下面的问题";
            } else {
                $problem_str = "请注意以下问题";
            }
            echo '<div class="alert">安装时' .$problem_str . ':';
    ...
    if (!isset($stype)) {
        echo '<p>要使用<a href="' . ENCODER_URL . '" target=encoder>ionCube PHP Encoder</a>编译的文件, 必须安装ionCube扩展.</p>';
    }

    if (!is_supported_php_version()) {
        echo '<p>你服务器上的PHP版本 ' . PHP_VERSION . ' 太低.  必须是PHP5以上.</p>';
	} elseif ($latest_supported_php_version = is_after_max_php_version_supported()) {
        echo '<p>你服务器上的PHP版本 ' . PHP_VERSION . ' 太高.  目前还不支持.</p>';
		//echo '<strong>你的PHP版本：' . PHP_VERSION . ' 太高. <br/>目前还不支持ionCube. '.</strong>';
        ...
        if ($loader == ERROR_WINDOWS_64_BIT) {
            echo '<p>windows 64位PHP不可用. 你可以 <b>安装 32位PHP</b> </p>';
            if ($sysinfo['THREAD_SAFE']) {
                echo '<li>下载以下一种文件(32-bit Windows x86 loaders):';
            } else {
                echo '<li>下载以下一种文件(32-bit Windows non-TS x86 loaders):';
            }
...
function server_selection_form()
{
    ...
    $form = <<<EOT
    <p>这个向导将会告诉你怎么安装ionCube Loader.</p>
    <script type=text/javascript>
        function trim(s) {
    ...
    <form method=GET action=$self>
        ...
        <input type=radio id=local name=stype value=l checked onclick="document.getElementById('hostinginfo').style.display = 'none';"><label for=local>Local install</label>
...
function heading()
{
	...
	<div id="important">
	<h3 class="important">重要: 安装完成后删除此文件</h3>
	</div>
    <div id="main">
