...
class Withdraw extends YZ
{
    ...
    public function index()
    {
        ...
        $result['json']['content']['withdraw'] = $result['json']['set']['withdraw'] > 0 ? "注意： 可用{$result['json']['set']['texts']['commission']}满 <span style='color:red'>{$result['json']['set']['withdraw']}</span> 元后才能申请{$result['json']['set']['texts']['withdraw']}" : "";
        ...
        $result['json']['content']['proportion_money'] = $result['json']['member']['proportion_money'] ? "注意： 您需在消费<span style='color:red'>{$result['json']['set']['proportion_money']}</span> 元后才能申请{$result['json']['set']['texts']['withdraw']}" : "";
