...
class Display extends YZ
{
    public function index()
    {
        ...
        $pindex = 1;//max(1, intval($_GPC['page']));
        $psize = 5;
        $status = Request::input("status");//$_GPC['status'];
        ...
        foreach ($list as $key => &$row) {
            ...
            if($p_cashier){
                $row['name'] = set_medias(pdo_fetch('select cs.name,cs.thumb from ' .tablename('sz_yi_cashier_store'). 'cs '.'left join ' .tablename('sz_yi_cashier_order'). ' co on cs.id = co.cashier_store_id where co.order_id=:orderid and co.uniacid=:uniacid', array(':orderid' => $row['id'],':uniacid'=>$uniacid)), 'thumb');
            }
