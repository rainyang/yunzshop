                    ...
                    if (!empty($order['virtual']) && $pv) {
                        ...
                        if (p('channel')) {
                            if ($params['ischannelpay'] == 1) {
                                pdo_query('update ' . tablename('sz_yi_order') . " set status=3, paytime=".time().", finishtime=".time().", payordersn=ordersn_general, ordersn_general=ordersn where {$order_update} and uniacid='{$uniacid}' ");
                            } else {
                                pdo_query('update ' . tablename('sz_yi_order') . " set status=1, paytime=".time().", payordersn=ordersn_general, ordersn_general=ordersn where {$order_update} and uniacid='{$uniacid}' ");
                            }
                        } else {
                            pdo_query('update ' . tablename('sz_yi_order') . " set status=1, paytime=".time().", payordersn=ordersn_general, ordersn_general=ordersn where {$order_update} and uniacid='{$uniacid}' ");
                        }
