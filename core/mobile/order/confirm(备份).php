...
if ($_W['isajax']) {
    ...
    if ($operation == 'display') {
        ...
        foreach ($goods as &$g) {
            ...
            if ($pindiana && $_GPC['isindiana']) {
                ...
                $indiana = set_medias(pdo_fetch("SELECT ig.*, g.thumb, ip.period, ip.shengyu_codes, ip.zong_codes, ip.period_num, ip.init_money as initmoney FROM " . tablename('sz_yi_indiana_goods') . " ig 
                   ...
                   $indiana['initmoney'] = number_format($indiana['initmoney'],2);
                   $indiana['shengyu'] = $indiana['shengyu_codes']/$indiana['zong_codes']*100;
            }

            $g['indiana'] = $indiana;
            $order_all[$g['supplier_uid']]['indiana_realprice'] += number_format($indiana['initmoney'] * $g['total'],2);
            $order_all[$g['supplier_uid']]['marketprice'] += $g['marketprice'];
        ...
        foreach ($suppliers as $key => $val) {
            ...
            $indiana_total += number_format($order_all[$val['supplier_uid']]['indiana_realprice'],2);
        ...
        show_json(1, array(
            ...
            'indiana_total' => number_format($indiana_total, 2),
            'address' => $address,
            //'carrier' => $carrier,
