...
#      Version: 0.0.1
#   LastChange: 2016-02-13 01:14:27
#      History:
=============================================================================*/
if (!defined('IN_IA')) {
    exit('Access Denied');
...
if ($_W['isajax']) {
	if ($operation == 'display') {
		...
		$data = pdo_fetch('select id from ' . tablename('sz_yi_member_cart') . ' where openid=:openid and goodsid=:id and  optionid=:optionid and deleted=0 and  uniacid=:uniacid   limit 1', array(':uniacid' => $uniacid, ':openid' => $openid, ':optionid' => $optionid, ':id' => $id));
		$cartcount = pdo_fetchcolumn('select count(id) from ' . tablename('sz_yi_member_cart') . ' where openid=:openid and deleted=0 and uniacid=:uniacid limit 1', array(':uniacid' => $uniacid, ':openid' => $openid));
		if (empty($data)) {
