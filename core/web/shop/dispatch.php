...
if ($operation == 'display') {
    ...
    if (!empty($_GPC['displayorder'])) {
        ...
        foreach ($_GPC['displayorder'] as $id => $displayorder) {
            pdo_update('sz_yi_dispatch', array(
    ...
    $list = pdo_fetchall("SELECT * FROM " . tablename('sz_yi_dispatch') . " WHERE uniacid = '{$_W['uniacid']}' ORDER BY id asc");
    ...
    if (checksubmit('submit')) {
        ...
        if (!empty($id)) {
            ...
            pdo_update('sz_yi_dispatch', $data, array(
                'id' => $id
            ));
        } else {
    ...
    pdo_delete('sz_yi_dispatch', array(
        'id' => $id
    ));
