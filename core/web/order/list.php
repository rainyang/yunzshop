...
if ($operation == "display") {
    ...
    $totals['status4'] = pdo_fetchcolumn('SELECT COUNT(DISTINCT 1) FROM ' . tablename('sz_yi_order') . ' o ' . ' left join ' . tablename('sz_yi_order_refund') . ' r on r.orderid= o.id' . ' left join ' . tablename('sz_yi_member') . ' m on m.openid=o.openid  and m.uniacid =  o.uniacid' . ' left join ' . tablename('sz_yi_member_address') . ' a on o.addressid = a.id ' . ' left join ' . tablename('sz_yi_member') . ' sm on sm.openid = o.verifyopenid and sm.uniacid=o.uniacid' . ' left join ' . tablename('sz_yi_saler') . ' s on s.openid = o.verifyopenid and s.uniacid=o.uniacid' . " WHERE o.uniacid=:uniacid AND o.refundtime=0 AND o.refundid<>0 and r.status=0  and o.refundstate>=0 and o.deleted=0 {$supplier_conds}" . $join_order_type, array(':uniacid' => $_W['uniacid']));    $totals['status5'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('sz_yi_order') . '' . ' WHERE ' . $condition . ' and refundtime<>0 AND refundid<>0' . $supplier_cond, $paras);
    if(p('hotel') && $type=='hotel'){
...
function order_list_refund($item)
{
    ...
    if ($refundstatus == 0) {
        ...
        if ($item['paytype'] == 1) {
            ...
            //exit;
        }else if ($item['paytype'] == 26 || $item['paytype'] == 25) {
