...
if ($operation == "display") {
    ...
    if(p('supplier')){
        if($perm_role == 1){
             ...
             **/
            $costmoney = pdo_fetchcolumn(' select ifnull(sum(IF(go.costprice,go.costprice,g.costprice)*og.total),0) from ' . tablename('sz_yi_order_goods') . ' og left join ' . tablename('sz_yi_order') . ' o on o.id=og.orderid left join ' . tablename('sz_yi_goods') . ' g on g.id=og.goodsid left join ' . tablename('sz_yi_goods_option') . ' go on go.goodsid=g.id where og.supplier_uid=:supplier_uid and og.supplier_apply_status!=1 and o.status=3 and og.uniacid=:uniacid',array(
                    ':supplier_uid' => $_W['uid'],
                    ':uniacid' => $_W['uniacid']
                ));
