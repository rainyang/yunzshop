...
//供应商增加
//if(pdo_tableexists(tablename('sz_yi_af_supplier'))){
...
//供应商提现字段int不对要改
if(pdo_tableexists(tablename('sz_yi_supplier_apply'))){
    if(!pdo_fieldexists('sz_yi_supplier_apply', 'apply_money')) {
      pdo_query("ALTER TABLE ".tablename('sz_yi_supplier_apply')." ADD `apply_money` DECIMAL( 10, 2 ) NOT NULL DEFAULT '0.00' COMMENT '申请提现金额';");
    }
    if(pdo_fieldexists('sz_yi_supplier_apply', 'apply_money')) {
      pdo_query("ALTER TABLE ".tablename('sz_yi_supplier_apply')." CHANGE `apply_money` `apply_money` DECIMAL( 10, 2 ) NOT NULL DEFAULT '0.00' COMMENT '申请提现金额';");
    }
}
...
if(!pdo_fieldexists('sz_yi_notice', 'desc')) {
  pdo_query("ALTER TABLE ".tablename('sz_yi_notice')." ADD `desc` VARCHAR( 255 ) DEFAULT '';");
}
